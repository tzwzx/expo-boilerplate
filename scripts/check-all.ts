...
): string => ((endTime - startTime) / millisecondsPerSecond).toFixed(2);
...
const printFailedCommand = (
  command: string,
  error: CommandExecutionError
): void => {
  console.error(`❌ Command failed: ${command}`);
  printCommandOutput(error.stdout, error.stderr);
};

const createSequentialError = (
  errors: { command: string; error: Error }[]
): Error => {
  const errorMessages = errors
    .map((error) => `${error.command}: ${error.error.message}`)
    .join("\n");
  return new Error(`Sequential commands failed:\n${errorMessages}`);
};
...
const getTimerResult = (
  ...
  getDurationSeconds(startTime, performance.now(), millisecondsPerSecond);

const exitWithTimerMessage = (
  isSuccess: boolean,
  message: string,
  duration: string
): never => {
  if (isSuccess) {
    console.log(`✅ ${message} (execution time: ${duration}s)\n`);
    process.exit(0);
  }

  console.error(`❌ ${message} (execution time: ${duration}s)`);
  process.exit(1);
};
...
const runSequential = async (commands: readonly string[]): Promise<void> => {
  ...
  for (const command of commands) {
    try {
      ...
      printFailedCommand(command, execError);
      errors.push({ command, error: execError });
    }
  }

  if (errors.length > 0) {
    throw createSequentialError(errors);
  }
};
...
const runWithTimer = async (
  ...
  try {
    await fn();
    exitWithTimerMessage(
      true,
      successMessage,
      getTimerResult(startTime, MILLISECONDS_PER_SECOND)
    );
  } catch {
    exitWithTimerMessage(
      false,
...
const runSelectedMode = (isFix: boolean): Promise<void> =>
  isFix ? runFix() : runCheck();
